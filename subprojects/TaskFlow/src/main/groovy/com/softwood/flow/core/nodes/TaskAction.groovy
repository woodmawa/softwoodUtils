...
class TaskAction extends AbstractFlowNode{
    ...
    static newAction (name = null,  Closure closure) {
        /*
        ...
        def ta = new TaskAction(ctx: ctx, name: name ?: "anonymous", action:cloned)
    ...
    static newAction (name = null,  long delay, Closure closure) {
        def ctx
        def owner = closure.owner
        def delegate = closure.delegate
        if (owner instanceof Closure &&
    ...
    private def actionTask (AbstractFlowNode previousNode, AbstractFlowNode step,  Object[] args, Closure errHandler = null) {
        try {
            ...
            promise.whenBound {
                ...
                step.ctx?.flowListeners.each { listener ->
                    ...
                    if(ctx.type = FlowType.Process) {
                        ...
                        listener.flowEventUpdate(ctx.flow, fe)
                    }
                }
    ...
    def  then (AbstractFlowNode nextStep, args = null, Closure errHandler = null) {
        ...
        log.debug "then : fire <$name> action "
